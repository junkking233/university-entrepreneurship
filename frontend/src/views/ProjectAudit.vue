  ...
  <div class="project-audit-page">
    ...
    <el-dialog v-model="detailVisible" title="项目详情" width="700px">
      <el-descriptions :column="2" border>
        ...
        <el-descriptions-item label="创建者">{{ currentItem.founder }}</el-descriptions-item>
        ...
        <el-descriptions-item label="融资金额">{{ currentItem.fundingTarget }}万</el-descriptions-item>
...
async function handleAudit(row, status) {
  try {
    await auditProject(row.id, { status })
...
async function submitReject() {
  ...
  await handleAudit(currentItem.value, 'rejected')
...
async function fetchList() {
  ...
  try {
    ...
    projectList.value = [
      { id: 1, title: '智能校园助手', founder: '张三', category: '人工智能', status: 'pending', fundingTarget: 50, description: '基于AI的校园服务平台', createdAt: '2024-05-20' },
      { id: 2, title: '在线教育平台', founder: '李四', category: '教育科技', status: 'pending', fundingTarget: 80, description: '在线互动教学平台', createdAt: '2024-05-22' }
    ]
    pagination.total = 2
  } finally {
    loading.value = false
